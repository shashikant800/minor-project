...
class CMD(object):
    ...
    def evaluate(self, loader, eval_dep=False, decode_type='mbr', model=None):
        ...
        supports_dep_eval = None  # None means not checked yet
        for x, y in t:
            ...
            if eval_dep:
                if supports_dep_eval is None:
                    # Check once if model supports dependency evaluation
                    supports_dep_eval = 'prediction_arc' in result
                    if not supports_dep_eval:
                        print("Warning: Model does not support dependency evaluation (prediction_arc not found). Skipping UAS metric.")
                if supports_dep_eval:
        ...
        if not eval_dep:
            ...
            if supports_dep_eval:
                ...
                # Return None for UAS if not supported
                return metric_f1, None, metric_ll
